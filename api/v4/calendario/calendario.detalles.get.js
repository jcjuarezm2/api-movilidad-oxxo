...
exports.detalles_get = (req, res, next) => {
  ...
  switch(tipo_consulta) {
      case "D":
        ...
        if(isGreather) {
            fecha_consulta = valor = fecha_token;
        }
  ...
  if (err) {
   ...
   if(result[0].respuestas>0) {
      ...
      `xxbdo_checklists_tiendas`.`titulo_app` AS `version`, \
      ...
      `xxbdo_observaciones`.`pendiente_agregado`, \
      ...
      AND `xxbdo_checklists_tiendas`.`xxbdo_checklists_id`=`xxbdo_respuestas`.`xxbdo_checklists_id` \
      ...
      //AND `xxbdo_respuestas`.`respuesta` IN ("+consulta_respuesta+") \
       ...
       switch(tipo_consulta) {
          ...
          `xxbdo_areas`, \
          `xxbdo_estandares` \
          WHERE `xxbdo_checklists_tiendas`.`xxbdo_checklists_id` = ( \
          ...
          `xxbdo_estandares` \
          WHERE `xxbdo_checklists_tiendas`.`xxbdo_checklists_id` = ( \
              SELECT `xxbdo_checklists_tiendas`.`xxbdo_checklists_id` \
              FROM `xxbdo_checklists_tiendas` \
              WHERE `xxbdo_checklists_tiendas`.`cr_plaza`='"+crplaza+"' \
              AND `xxbdo_checklists_tiendas`.`cr_tienda`='"+crtienda+"' \
              AND '"+valor+"' \
              BETWEEN `xxbdo_checklists_tiendas`.`fecha_inicio` \
              AND IFNULL(`xxbdo_checklists_tiendas`.`fecha_fin`, '"+fecha_token+"') \
              LIMIT 1 \
          ) \
          AND `xxbdo_areas_estandares`.`id` IN( \
              SELECT `xxbdo_tiendas_has_areas_estandares`.`xxbdo_areas_estandares_id` \
          ...
          AND `xxbdo_checklists_tiendas`.`cr_plaza`='"+crplaza+"' \
          AND `xxbdo_checklists_tiendas`.`cr_tienda`='"+crtienda+"' \
          AND `xxbdo_checklists_tiendas`.`activo`=1 "+tipo_condition;
          entries = [valor];
          stmt="SELECT * FROM (\
              "+sql_stds+" \
              UNION \
   ...
   qry=db.query(stmt, entries, (err, resultB) => {
      ...
      results = formatResults(
          req.calendario_roles_en_tienda,
          resultB, 
          tipo_consulta, 
          existe,
          //valor,
...
function formatResults(roles, 
    rows, 
    tipo_consulta, 
    existe_dia, 
    year, 
    num_week_or_month=null, 
    titulo=null, 
    subtitulo=null) {
    if(rows) {
