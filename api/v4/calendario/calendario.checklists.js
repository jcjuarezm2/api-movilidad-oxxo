...
exports.checklists = (req, res, next) => {
    ...
    qry = db.query(stmt, data, (err, rst) => {
        if(dbg) console.log("[CC:68] Get checklist from current date: ", qry.sql);
        if (err) {
            err.status = 500;
            return next(err);
        }
        ...
        if(rst) {
            ...
            req.version_estandares_id = rst[0].veid;
            ...
            if(dbg) console.log("[CC:80] checklist from current date =", req.checklist_id, req.checklist_inicio, req.checklist_fin);
            ...
            qry = db.query(stmt, data, (err, result) => {
                if(dbg) console.log("[CC:102] ", qry.sql);
                if (err) {
                    err.status = 500;
                    return next(err);
                }
