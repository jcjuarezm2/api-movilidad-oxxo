...
exports.matriz_tiendas = (req, res, next) => {
    if(dbg) console.log("[8] Start resumen calendario...");
    if(!db) {
    ...
    let fecha_consulta = req.fecha_consulta_fin || 
                         bdoDates.getBDOFormat(fecha_token, 'YYYY-MM-DD');
    let consulta_checklist_id = req.body.chk || null;
    let cr_plaza = req.resumenbdo_plaza;
    let tiendas = req.resumenbdo_tiendas;
    let request_continue = true;
    let error_message = "";
    let num_interval = req.num_total_dias; //days 7
    ...
    if(request_continue) {
      ...
      [req_inicio, 
        ...
        if(resumen_data) {
            ...
            req.resumenbdo_fecha_inicio_alertas = req_inicio;
            req.resumenbdo_fecha_fin_alertas = req_fin;
            req.resumenbdo_headers_alertas = resumen_headers;
            req.resumenbdo_matriz_tiendas_alertas = matriz_tiendas;
            req.resumenbdo_plaza_alertas = cr_plaza;
            req.resumenbdo_tiendas_alertas = tiendas;
            next();
        } else {
            const error  = 
