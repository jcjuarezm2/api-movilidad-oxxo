-- query para reporte de uso semanal de la app:
...
`module` AS `MODULO`, 
SUBSTRING_INDEX(`activity`, 'Activity', 1) AS `PANTALLA`, 
`action` AS `ACCION`, 
`method` AS `METODO`, 
`user` AS `USUARIO`, 
DATE(`creation`) as `FECHA`, 
TIME(`creation`) AS `HORA`, 
CONCAT(REPLACE(`app_version`,'release',''),'release') AS `APP_VERSION` 
FROM `xxmet_logs` 
WHERE DATE(`creation`) 
BETWEEN '2020-02-17' 
AND '2020-02-23' 
ORDER BY `cr_plaza`, `cr_tienda`, `creation` DESC;​

-- 2020-02-26
-- query nuevo para reporte de uso semanal de la app sin ORDER BY:
SELECT `cr_plaza` AS `PLAZA`, 
`cr_tienda` AS `TIENDA`, 
`device_name` AS `DISPOSITIVO`, 
`device_type` AS `TIPO`, 
...
FROM `xxmet_logs` 
WHERE `creation` 
BETWEEN '2020-02-01 00:00:00' 
AND '2020-02-01 23:59:59';


-- query para reporte de captura de checklist por rango de fecha y plaza:
...
WHERE `xxbdo_respuestas`.`cr_plaza`='10MDA'
AND `xxbdo_respuestas`.`año`='2019'
...
AND DATE(`xxbdo_respuestas`.`fecha_creacion`) BETWEEN '2019-05-31' AND '2019-07-31'
ORDER BY `xxbdo_respuestas`.`cr_plaza`,
`xxbdo_respuestas`.`cr_tienda`,
`xxbdo_respuestas`.`tipo`,           
`xxbdo_respuestas`.`fecha_creacion`;
...
AND `xxbdo_areas`.`xxbdo_version_estandares_id`=`xxbdo_checklists`.`xxbdo_version_estandares_id`       
AND `xxbdo_estandares`.`xxbdo_version_estandares_id`=`xxbdo_checklists`.`xxbdo_version_estandares_id`  
AND `xxbdo_respuestas`.`fecha_creacion` BETWEEN '2020-01-01 00:00:00' AND '2020-01-01 23:59:59';

-- nuevo query para reporte de captura de checklist por rango de fecha y plaza, sin ORDER BY:
CREATE TABLE xxbdo_reporte_bdo_10ctl_Semana_10_2020 
SELECT `xxbdo_checklists`.`titulo_app` AS `version_bdo`,       
`xxbdo_respuestas`.`cr_plaza`,
`xxbdo_respuestas`.`cr_tienda`,
`xxbdo_areas`.`titulo` AS `area_titulo`,       
...
WHERE `xxbdo_respuestas`.`cr_plaza`='10CTL'
AND `xxbdo_respuestas`.`año`='2020'
AND `xxbdo_areas_estandares`.`id`=`xxbdo_respuestas`.`xxbdo_areas_estandares_id`       
AND `xxbdo_estandares`.`id`=`xxbdo_areas_estandares`.`xxbdo_estandares_id`       
AND `xxbdo_areas`.`id`=`xxbdo_areas_estandares`.`xxbdo_areas_id`       
...
AND `xxbdo_areas`.`xxbdo_version_estandares_id`=`xxbdo_checklists`.`xxbdo_version_estandares_id`       
AND `xxbdo_estandares`.`xxbdo_version_estandares_id`=`xxbdo_checklists`.`xxbdo_version_estandares_id`  
AND `xxbdo_respuestas`.`fecha_creacion` BETWEEN '2020-03-01 00:00:00' AND '2020-03-09 23:59:59';

-- query para quitar tabs y espacios en registro de observaciones:
UPDATE `xxbdo_observaciones` 
SET `descripcion`=REPLACE(TRIM(descripcion), '\t', ''), 
`causa`=REPLACE(TRIM(causa), '\t', ''), 
`accion`=REPLACE(TRIM(accion), '\t', ''), 
`accion_responsable`=REPLACE(TRIM(accion_responsable), '\t', ''),
`observacion`=REPLACE(TRIM(observacion), '\t', '');
