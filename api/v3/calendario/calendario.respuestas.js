...
exports.respuestas_get = (req, res, next) => {
    ...
    qry=db.query(stmt, data, (err, rst) => {
        ...
        if(dbg) console.log("[CR:214] Call getReportData(fecha_consulta, tipo_consulta, num_interval): ", fecha_consulta, tipo_consulta, num_interval);
        ...
        if(dbg) console.log("[CR::235] Bitacora dias = " + JSON.stringify(bitacora_dias)); 
        if(dbg) console.log("[CR::236] Bitacora semanas = " + JSON.stringify(bitacora_semanas)); 
        if(dbg) console.log("[CR::237] Bitacora meses = " + JSON.stringify(bitacora_meses)); 
        //
        let results  = null,
...
function getResultsData(rows, valor=null, tipo_headers=null) {
   ...
   if(dbg) console.log("[CR:425] End getResultsData.");
