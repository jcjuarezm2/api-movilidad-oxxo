...
exports.roles_en_tienda = (req, res, next) => {
    if(dbg) console.log("[CR:8] Start roles_en_tienda ...");
    if(!db) {
    ...
    stmt="SELECT valor FROM `xxbdo_configuracion` WHERE modulo='pendientes' AND parametro='numero_dias_visibles_hacia_atras_fecha_seleccionada'";
    
    qry=db.query(stmt, null, (err, rst) => {
        if(dbg) console.log("[27] Get pendientes.numero_dias_visibles_hacia_atras_fecha_seleccionada = ", qry.sql);
        if (err) {
            err.status=500;
            return next(err);
        }

        pendientes_rango_listado_en_dias = rst[0].valor || 0;
        ...
        if(pendientes_rango_listado_en_dias) {
            ...
            qry=db.query(stmt, null, (err, rows) => {
                if(dbg) console.log("[43] Get roles_en_tienda = ", qry.sql);
                if (err) {
                    err.status = 500;
                    return next(err);
                }
