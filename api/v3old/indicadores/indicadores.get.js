...
const dbg = false;
  ...
  exports.indicadores_get = (req, res, next) => {
    ...
    if(ver_checklist_id) {
      ...
      fecha_consulta = req.checklist_fin;
    } else if(valor) {
    ...
    let fecha_fin_checklist = req.checklist_fin || valor;
    ...
    if(dbg) console.log("[IG:64] valor a consultar = " + valor);
    ...
    if(result[0].respuestas>0) {
      ...
      if(dbg) console.log("[IG:219] Respuestas not found!");
      if(ver_checklist_id) {
        if(dbg) console.log("IG:[221] Search for checklist_id: ", ver_checklist_id);
        ...
        sql_checklist = "`xxbdo_checklists_tiendas`.`xxbdo_checklists_id` = ( \
          ...
          AND IFNULL(`fecha_fin`, '"+fecha_fin_checklist+"') \
          LIMIT 1 \
      )";//fecha_token
    ...
    qry=db.query(sql, entries, (err, rows) => {
      if(dbg) console.log("[IG:337] ", qry.sql);
      if(err) {
          err.status = 500;
          return next(err);
      }
    ...
    function formatResults(rows, 
      ...
      checklists_data
        ...
        if(rows || checklist_id) {
          let results = [];
          let plantilla = null;
          let is_current = false; //null;
          if(!rows) {
            rows = [];
          }
          if(rows.length>0 || checklist_id) {
              let areas=[], nrow=0;
              rows.forEach(row => {
  ...
  exports.indicadores_ayuda_get = (req, res, next) => {
      ...
      if (errors) {
         ...
         qry = db.query(stmt, entries, (err, rows) => {
              if(dbg) console.log("[IG:492] ", qry.sql);
              if (err) {
                  err.status = 500;
                  return next(err);
              }
