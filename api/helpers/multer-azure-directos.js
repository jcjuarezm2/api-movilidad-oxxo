...
var uploadMulterAzure = multer({
  storage: multerAzure({
    ...
    container: app_configuration.get('azure.sas.blob.containers.directos.name'), // '[Blob Container Name]'  //Any container name, it will be created if it doesn't exist
    blobPathResolver: function (req, file, callback) {
      const name = Date.now()
      const transaccion = req.params.transaccion;
      const ext = file.mimetype === 'image/jpeg' || file.mimetype === 'image/png' ? 'png' : 'pdf'
      var blobPath = `${transaccion}/${name}.${ext}`
