...
dbg               = false;
  ...
  exports.indicadores_get = (req, res, next) => {
    ...
    sql_checklist = "";
    if(ver_checklist_id) {
      sql_checklist = "AND `xxbdo_checklists`.`id`='"+ver_checklist_id+"'";
    }
    if(result[0].respuestas>0) {
      ...
      "+sql_checklist+"  \
      AND `xxbdo_respuestas_indicadores`.`xxbdo_checklists_id`=`xxbdo_checklists`.`id` \
      ...
      if(ver_checklist_id) {
        ...
        sql_checklist="`xxbdo_checklists`.`id` =?";
