...
exports.checklist_put = (req, res, next) => {
    ...
    putBdoDetalles(rst_array[0], usuario, ip, function(results) {
        ...
        if(dbg) console.log("[69] Send response 200 - OK.");
        res.status(200).json(rst_array[2]);
    });
        ...
        items.forEach(function(item) {
            ...
            switch(item[0]) {
                case "respuesta":
                    ...
                    fecha_modificacion=? \
                    WHERE id=?";
                    data = [item[2], usuario, ip, timestamp, item[1]];
                break;
                case "observacion":
    ...
    respuestas.forEach(respuesta => {
        ...
        if(respuesta.obs) {
            if(respuesta.obs.id) {
                ...
                update_registros.push(res);
                rel={"res":respuesta.obs.eid, "obs":respuesta.obs.id};
            } else {
                //insert observacion
                uuid_obs  = uuidv4();
                timestamp = bdoDates.getBDOCurrentTimestamp();
                ...
                res = [
                    ...
                    rel={"res":respuesta.obs.eid, "obs":uuid_obs};
            }
            respuesta_observacion.push(rel);
        }
    });
    return [update_registros, insert_observaciones, respuesta_observacion];
}
